...
fn test_report_similarity() {
    ...
    r2.target = Content::File(Source::RawFile(SourceFile::Local(4, "/proc/test".into())));
    ...
    r2.log_reports.push(LogReport {
        ...
        source: Source::RawFile(SourceFile::Local(4, "/proc/cmd".into())),
    });
    ...
    let default_source = Source::TarFile(
        std::sync::Arc::new(SourceFile::Local(4, "/tmp/test.tar.xz".into())),
        "entry.txt".into(),
