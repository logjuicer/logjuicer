...
class Api(object):
    ...
    def list(self):
        ...
        with self.db.session() as session:
            for anomaly in session.query(model.Anomaly).order_by(
                model.Anomaly.report_date.desc()
            ):
                results.append(
                    {
                        "uuid": anomaly.uuid,
    ...
    def _getAnomaly(self, session, anomaly_id):
        try:
            return session.query(model.Anomaly).filter_by(uuid=anomaly_id).one()
                ...
                if os.path.exists(logpath):
                    ...
                    try:
                        ...
                        while True:
                            ...
                            if line == b"":
                                break
                            loglines.append(line.decode("ascii", errors="ignore"))
                    except Exception:
                    ...
                    self.rpc.download(
                        os.path.join(anomaly.build.log_url, logfile["path"]), logpath
                    )
...
class StaticHandler(object):
    ...
    def default(self, path):
        ...
        handled = cherrypy.lib.static.staticdir(
            section="", dir=self.root, index="index.html"
        )
        if not handled:
            # When not found, serve the index.html
            return cherrypy.lib.static.serve_file(
                path=os.path.join(self.root, "index.html"), content_type="text/html"
            )
...
class ServerWorker(rpc.Listener):
        ...
        if result.get("report"):
            ...
            try:
                ...
                self.history.appendleft(
                    "%s: new report added: %s" % (request["uuid"], aid)
                )
    ...
    def archive(self, anomaly):
        ...
        def resolvePath(build, obj):
            ...
            return os.path.join(
                self.api.logserver_folder, urlsplit.netloc, urlsplit.path[1:], obj.path
            )
        ...
        for logfile in anomaly.logfiles:
            ...
            for baseline in logfile.model.baselines:
                ...
                baselines.append({"local_path": resolvePath(baseline.build, baseline)})
            logfiles.append(
                {
                    "name": logfile.path.replace("/", "_"),
                    "local_path": resolvePath(anomaly.build, logfile),
                    "scores": [[line.nr, line.confidence] for line in logfile.lines],
                    "baselines": baselines,
                }
            )
    ...
    def do_archive(self, archive):
        ...
        if not result.get("error"):
            try:
                ...
                with self.api.db.session() as session:
                    anomaly = session.query(model.Anomaly).filter_by(uuid=uuid).one()
    ...
    def handle_archive(self, infos, logfiles):
        ...
        for logfile in logfiles:
            ...
            json.dump(logfile["scores"], open(os.path.join(folder, "scores.json"), "w"))
            xsymlink(logfile["local_path"], os.path.join(folder, "targetlog.txt"))
            idx = 1
